...
### 3-1. 먼저 체리에 단체를 개설해야 합니다

2026년 9월 기준으로 위드프렌즈는 아직 체리에 등록되어 있지 않습니다.
아래 절차를 마쳐야 연결할 주소가 생깁니다.

| 순서 | 할 일 |
| --- | --- |
| 1 | 체리 **나눔단체 개설 신청** 구글폼 제출 — https://forms.gle/pemyTPvE2BJzBCNF9 |
| 2 | 법인 등기부등본 · 고유번호증 · 법인 통장 제출 후 이포넷과 금융 서비스 계약 |
| 3 | 이포넷이 위드프렌즈 전용 페이지를 개설 → 이때 주소가 나옵니다 |

문의: 02-565-8606 / support@cherry.charity

### 3-2. 받은 주소를 넣습니다

`data/site.js` 를 열고, 파일 위쪽의 아래 한 줄을 찾아 따옴표 안에 넣습니다.

```js
var CHERRY_DONATION_URL = "https://cherry.charity/public/group/2000000000000000000";
```

저장하고 올리면 **사이트 전체의 후원 버튼이 한 번에** 연결됩니다. 다른 파일은 고칠 필요가 없습니다.

체리 주소 형식은 두 가지입니다.

| 종류 | 형식 | 언제 쓰나 |
| --- | --- | --- |
| 단체 페이지 | `https://cherry.charity/public/group/<단체ID>` | 보통 이것을 씁니다. 여러 모금함을 한곳에 보여 줍니다 |
| 캠페인 페이지 | `https://cherry.charity/public/campaign/cmpgnDtlPage/<캠페인ID>` | 특정 캠페인 하나에 집중해 모금할 때 |

**비워 두면**: 모든 후원 버튼이 눌리지 않게 잠기고 "체리 기부 페이지 주소 준비 중" 안내가 나옵니다.
잘못된 주소로 이동하는 사고가 나지 않습니다.

> ⚠️ **주소나 단체ID를 추측해서 넣지 마세요.**
...
| 항목 | 위치 |
| --- | --- |
| 체리 기부 페이지 주소 (단체 개설 후 발급) | `data/site.js` → `CHERRY_DONATION_URL` |
